...
class SendWelcomeHandler(BaseHandler):
    async def handle(self, event, state, context=None):
        ...
        await event.answer("👋 Вы успешно прошли авторизацию и теперь вам доступен основной функционал системы! Вы можете продолжить перейдя в главное меню по кнопке ниже:", reply_markup=MainMenuKeyboard.go_to_main_menu_keyboard())