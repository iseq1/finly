import { renderPaginationControls } from '/static/js/income/render_transaction_pagination_history_table.js';
import { fetchIncomeTransactions } from '/static/js/income/fetch_transaction.js';
...
export async function renderIncomeTableWithPagination({
  start_date,
  end_date,
  page = 1,
  per_page = 10,
  ...
  const response = await fetchIncomeTransactions(params);
...
export function setupTableSorting() {
  ...
  headers.forEach(header => {
    ...
    header.addEventListener('click', async () => {
      ...
      await renderIncomeTableWithPagination({
        ...range,
        page: 1,
        per_page: 10,
